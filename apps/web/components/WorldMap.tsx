    ...
    const animate = () => {
      ...
      for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
          ...
          const distToHighlight1 = Math.sqrt(
            (hexX - highlight1X) ** 2 + (hexY - highlight1Y) ** 2
          );
          const distToHighlight2 = Math.sqrt(
            (hexX - highlight2X) ** 2 + (hexY - highlight2Y) ** 2
          );

          const distToCenter = Math.sqrt((hexX - centerX) ** 2 + (hexY - centerY) ** 2);

          const isHighlighted =
            (distToHighlight1 < 100 || distToHighlight2 < 80) &&
            distToCenter > minDistanceFromCenter;
